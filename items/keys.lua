...
module("items.keys", package.seeall())

-- UPDATE common SET com_script='items.keys' WHERE com_itemid IN (2121,2122,2123,2124,2141,2144,2145,2161,2556,2558,3054,3055,3056);
...
function UseItemWithCharacter(User,SourceItem,TargetChar,Counter,Param)
	...
	if SourceItem.data>0 and SourceItem.data<=3 and User:isInRangeToPosition(prisonPosition[SourceItem.data],3) then 
          ...
          base.common.InformNLS( User, "Du scheiterst dabei, den Gauner zu verurteilen. Dies ist nur mit gefesselten Strolchen im örtlichen Gericht möglich.", "You failed to sentence the thug. This works only with tied up rogues at the local court."); 
     end
