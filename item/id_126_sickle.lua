...
function UseItem( User, SourceItem, TargetItem, Counter, Param, ltstate )
    ...
    if ((TargetItem == nil) or (TargetItem.id == 0)) then
        TargetItem = BC_GetFrontItem( User );
    end
    
    if not TargetItem then
        User:inform("Hier kannst du nichts finden.", "You can't find anything here.");
        return;
    end
    ...
    for i, harvest in pairs(harvestList) do
        ...
        if groundok then
            ...
            if BC_ToolBreaks( User, SourceItem ) then -- Zange beschädigen
                BC_InformNLS( User,
            ...
            if not BC_FitForWork( User ) then
                return false;
            end
		...
		skilluse=1;
			...
			User:talkLanguage(CCharacter.say, CPlayer.german, "Deine Kenntnisse in der Kräuterkunde sind nicht ausreichend, um hier etwas Brauchbares zu sammeln.");
        	User:talkLanguage(CCharacter.say, CPlayer.english, "Your knowlege in herblore is not sufficiently to collect herbs here.");
            ...
            elseif ( harvest[ 4 ] == 0 ) then	
			if (ltstate == Action.none) then -- Untï¿½tig: Starte Kräutersammeln
