	...
	if (SourceItem.data == 666) then
		if (User:getSkill("ancient language") > 59) then
			...
			elseif (SourceItem.pos.z == 1) then
	            if (User:getPlayerLanguage()==0) then
	                User:inform("In deinen Hï¿½nden, Suchender, hï¿½lst du die zweite Weisheit. Unerbittlich und reinigend wie das Feuer ist auch die Weisheit. Wahre Weisheit brennt sich in unser Herz und schenkt uns Licht an dunklen Orten.");
	                User:inform("Die dritte Weisheit findet ihr in Findaris Umarmung ï¿½ber der tanzenden Ebene wo die Winde singen und die Berge uralte Geschichten flï¿½stern.");
	            else
	        ...
	        elseif (SourceItem.pos.z == 3) then
	            if (User:getPlayerLanguage()==0) then
	                User:inform("In deinen Hï¿½nden, Suchender, hï¿½lst du die dritte Weisheit. Wie wir die Luft zum Atmen brauchen, so ist auch die Weisheit ein unersetzliches Gut. Endlos singt sie uns ihr Lied.");
