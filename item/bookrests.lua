...
for i,player in ipairs(travler) do
	...
	local callpirates = function(dialog2)
	debug("..selected ")
...
--	debug("..selected ")
	local dialog2
	if User:getPlayerLanguage() == Player.german then
		dialog2 = SelectionDialog("Piraten Angriff", "Piraten nähern sich eurem Schiff, dir stehen folgende Möglichkeiten zur Wahl.", callpirates)
	else
		dialog2 = SelectionDialog("Pirates attack", "Pirates are coming closer to your ship. You can choose between following options.", callpirates)
	end
--	dialog:setCloseOnMove()
	
	for m=1,Amountoptions do
		dialog2:addOption(items[m],options[m])
	end
