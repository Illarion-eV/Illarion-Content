...
M.propertyTable = {
-- property name,DE name, position, default rent, keyID, doorID, town, required rank, required rank name, DE rank name
-- Some of the information is not used and was there for a fully automated version. Kept in case of future need.
--Cadomyr
 ...
 {"Flat Faith & Obedience", "Wohnung Glaube & Gehorsam",position(137,605,0),5000,2558,361, "Cadomyr",
 "4","Yeoman","Freisasse"},
 {"Flat Honour & Lineage", "Wohnung Ehre & Abstammung",position(138,603,1),5000,2558,362, "Cadomyr","4","Yeoman",
 "Freisasse"},
 {"Flat Truth & Justice", "Wohnung Ehrlichkeit & Gerechtigkeit",position(136,609,0),10000,2558,363, "Cadomyr","5",
 "Page","Page"},
...
{"Lucky Copper","Gläckskupfer",position(416,289,1),40000,2558,113, "Galmair","5","Merchant","Kaufmann"},
...
function checkIfLeaderOfTown(User, Town)
...
function getPropertyNameDE(Item)
    for i = 1, #M.propertyTable do
        if (Item.pos == M.propertyTable[i][3]) then
...
function propertyDeedLookAt(User, Item)
  local lookAt = ItemLookAt()
    lookAt.name = common.GetNLS(User, "", "Property Deed")
    lookAt.description = common.GetNLS(User, "", "The Property Deed of "..getPropertyName(Item)..".")
    return lookAt
end
...
function removeOwner(User, Item)
local propertyName = getPropertyName(Item)
...
ScriptVars:save()
User:inform(getText(User,"Vorheriger Mieter entfernt.","Previous renter removed."))
end
-- sets an owner
function setOwner(User, Item)
local property = getPropertyName(Item)
    local callback = function (dialog)
        ...
        local input = dialog:getInput()
            if (input == nil or input == '') then
                User:inform(getText(User,"Das Namenfeld darf nicht leer sein.","The name field can not be empty."))
            elseif checkIfOwnsProperty(input) then
                User:inform(getText(User,"Der Charakter mietet bereits ein Grundstäck.","Character already rents a\z
                 property."))
            else
                removeOwner(User, Item)
                ScriptVars:set("rentDuration"..property,1)
                ScriptVars:set("ownerof"..property,input)
                ScriptVars:save()
                User:inform(getText(User,input.."wurde als neuer Mieter eingetragen.",input.." set as new renter."))
                setSignature(User,Item)
            end
    end
    User:requestInputDialog(InputDialog(getText(User,"Mieter eintragen","Set Renter"), getText(User,"Trag hier den\z
    Namen des neuen Mieters ein.","Write in the name of who you want to set as the new renter."), false, 255, callback))
end
...
function setReqRank(User, Item)
local property = getPropertyName(Item)
local town = getTownName(Item)
    ...
    local success = dialogGalmair:getSuccess()
        if success then
            ...
            if selected == 1 then
                ...
                ScriptVars:save()
                User:inform(getText(User,"Der benötigte Rang wurde zu Patrizier gesetzt","Required rank has been \z
                 set to Patrician."))
            elseif selected == 2 then
                ScriptVars:set("nameEN"..property,"Financier")
                ScriptVars:set("nameDE"..property,"Finanzier")
                ScriptVars:save()
                User:inform(getText(User,"Der benötigte Rang wurde zu Finanzier gesetzt","Required rank has been \z
                 set to Financier."))
            elseif selected == 3 then
                ScriptVars:set("nameEN"..property,"Merchant")
                ScriptVars:set("nameDE"..property,"Kaufmann")
                ScriptVars:save()
                User:inform(getText(User,"Der benötigte Rang wurde zu Kaufmann gesetzt","Required rank has been \z
                 set to Merchant."))
            elseif selected == 4 then
                ScriptVars:set("nameEN"..property,"Grocer")
                ScriptVars:set("nameDE"..property,"Krämer")
                ScriptVars:save()
                User:inform(getText(User,"Der benötigte Rang wurde zu Krämer gesetzt","Required rank has been \z
                 set to Grocer."))
            elseif selected == 5 then
                ScriptVars:set("nameEN"..property,"Pedlar")
                ScriptVars:set("nameDE"..property,"Hausierer")
                ScriptVars:save()
                User:inform(getText(User,"Der benötigte Rang wurde zu Hausierer gesetzt","Required rank has been \z
                 set to Pedlar."))
            elseif selected == 6 then
                ScriptVars:set("nameEN"..property,"Assistant")
                ScriptVars:set("nameDE"..property,"Gehilfe")
                ScriptVars:save()
                User:inform(getText(User,"Der benötigte Rang wurde zu Gehilfe gesetzt","Required rank has been \z
                 set to Assistant."))
            else
                ScriptVars:set("nameEN"..property,"Tramp")
                ScriptVars:set("nameDE"..property,"Rumtreiber")
                ScriptVars:save()
                User:inform(getText(User,"Der benötigte Rang wurde zu Rumtreiber gesetzt","Required rank has been \z
                 set to Tramp."))
            end
        end
    end
    ...
    local success = dialogRunewick:getSuccess()
        if success then
            ...
            if selected == 1 then
                ...
                ScriptVars:save()
                User:inform(getText(User,"Der benötigte Rang wurde zu Dozent gesetzt","Required rank has been \z
                 set to Docent."))
            elseif selected == 2 then
                ScriptVars:set("nameEN"..property,"Doctor")
                ScriptVars:set("nameDE"..property,"Doktor")
                ScriptVars:save()
                User:inform(getText(User,"Der benötigte Rang wurde zu Doktor gesetzt","Required rank has been \z
                 set to Doctor."))
            elseif selected == 3 then
                ScriptVars:set("nameEN"..property,"Master")
                ScriptVars:set("nameDE"..property,"Magister")
                ScriptVars:save()
                User:inform(getText(User,"Der benötigte Rang wurde zu Magister gesetzt","Required rank has been \z
                set to Master."))
            elseif selected == 4 then
                ScriptVars:set("nameEN"..property,"Scholar")
                ScriptVars:set("nameDE"..property,"Gelehrter")
                ScriptVars:save()
                User:inform(getText(User,"Der benötigte Rang wurde zu Gelehrter gesetzt","Required rank has been \z
                 set to Scholar."))
            elseif selected == 5 then
                ScriptVars:set("nameEN"..property,"Student")
                ScriptVars:set("nameDE"..property,"Student")
                ScriptVars:save()
                User:inform(getText(User,"Der benötigte Rang wurde zu Student gesetzt","Required rank has been \z
                 set to Student."))
            elseif selected == 6 then
                ScriptVars:set("nameEN"..property,"Apprentice")
                ScriptVars:set("nameDE"..property,"Anwärter")
                ScriptVars:save()
                User:inform(getText(User,"Der benötigte Rang wurde zu Anwärter gesetzt","Required rank has been \z
                 set to Apprentice."))
            else
                ScriptVars:set("nameEN"..property,"Novice")
                ScriptVars:set("nameDE"..property,"Novize")
                ScriptVars:save()
                User:inform(getText(User,"Der benötigte Rang wurde zu Novize gesetzt","Required rank has been \z
                 set to Novice."))
            end
        end
    end
    ...
    local success = dialogCadomyr:getSuccess()
        if success then
            ...
            if selected == 1 then
                ...
                ScriptVars:save()
                User:inform(getText(User,"Der benötigte Rang wurde zu Ritter gesetzt","Required rank has been \z
                 set to Knight."))
            elseif selected == 2 then
                ScriptVars:set("nameEN"..property,"Squire")
                ScriptVars:set("nameDE"..property,"Knappe")
                ScriptVars:save()
                User:inform(getText(User,"Der benötigte Rang wurde zu Knappe gesetzt","Required rank has been \z
                 set to Squire."))
            elseif selected == 3 then
                ScriptVars:set("nameEN"..property,"Page")
                ScriptVars:set("nameDE"..property,"Page")
                ScriptVars:save()
                User:inform(getText(User,"Der benötigte Rang wurde zu Page gesetzt","Required rank has been \z
                 set to Page."))
            elseif selected == 4 then
                ScriptVars:set("nameEN"..property,"Yeoman")
                ScriptVars:set("nameDE"..property,"Freisasse")
                ScriptVars:save()
                User:inform(getText(User,"Der benötigte Rang wurde zu Freisasse gesetzt","Required rank has been \z
                 set to Yeoman."))
            elseif selected == 5 then
                ScriptVars:set("nameEN"..property,"Servant")
                ScriptVars:set("nameDE"..property,"Diener")
                ScriptVars:save()
                User:inform(getText(User,"Der benötigte Rang wurde zu Diener gesetzt","Required rank has \z
                 been set to Servant."))
            elseif selected == 6 then
                ScriptVars:set("nameEN"..property,"Bondsman")
                ScriptVars:set("nameDE"..property,"Höriger")
                ScriptVars:save()
                User:inform(getText(User,"Der benötigte Rang wurde zu Höriger gesetzt","Required rank has \z
                 been set to Bondsman."))
            else
                ScriptVars:set("nameEN"..property,"Serf")
                ScriptVars:set("nameDE"..property,"Leibeigener")
                ScriptVars:save()
                User:inform(getText(User,"Der benötigte Rang wurde zu Leibeigener gesetzt","Required rank \z
                has been set to Serf."))
            end
        end
    end
    local dialogGalmair = SelectionDialog(getText(User,"Benötigten Rang einstellen","Set required rank"),
    getText(User,"Bestimme welcher Rang benötigt werden soll, um dieses Grundstäck zu mieten.",
    "Choose which rank should be required to rent this property."), callback1)
    local dialogRunewick = SelectionDialog(getText(User,"Benötigten Rang einstellen","Set required rank"),
    getText(User,"Bestimme welcher Rang benötigt werden soll, um dieses Grundstäck zu mieten.",
    "Choose which rank should be required to rent this property."), callback2)
    local dialogCadomyr = SelectionDialog(getText(User,"Benötigten Rang einstellen","Set required rank"),
    getText(User,"Bestimme welcher Rang benötigt werden soll, um dieses Grundstäck zu mieten.",
    "Choose which rank should be required to rent this property."), callback3)
    dialogGalmair:addOption(0, getText(User,"Patrizier","Patrician"))
    dialogGalmair:addOption(0, getText(User,"Finanzier","Financier"))
    dialogGalmair:addOption(0, getText(User,"Kaufmann","Merchant"))
    dialogGalmair:addOption(0, getText(User,"Krämer","Grocer"))
    dialogGalmair:addOption(0, getText(User,"Hausierer","Pedlar"))
    dialogGalmair:addOption(0, getText(User,"Gehilfe","Assistant"))
    dialogGalmair:addOption(0, getText(User,"Rumtreiber","Tramp"))
    dialogRunewick:addOption(0, getText(User,"Dozent","Docent"))
    dialogRunewick:addOption(0, getText(User,"Doktor","Doctor"))
    dialogRunewick:addOption(0, getText(User,"Magister","Master"))
    dialogRunewick:addOption(0, getText(User,"Gelehrter","Scholar"))
    dialogRunewick:addOption(0, getText(User,"Student","Student"))
    dialogRunewick:addOption(0, getText(User,"Anwärter","Apprentice"))
    dialogRunewick:addOption(0, getText(User,"Novize","Novice"))
    dialogCadomyr:addOption(0, getText(User,"Ritter","Knight"))
    dialogCadomyr:addOption(0, getText(User,"Knappe","Squire"))
    dialogCadomyr:addOption(0, getText(User,"Page","Page"))
    dialogCadomyr:addOption(0, getText(User,"Freisasse","Yeoman"))
    dialogCadomyr:addOption(0, getText(User,"Diener","Servant"))
    dialogCadomyr:addOption(0, getText(User,"Höriger","Bondsman"))
    dialogCadomyr:addOption(0, getText(User,"Leibeigener","Serf"))
...
function setRent(User, Item)
local propertyName = getPropertyName(Item)
local rentEN = getRent(Item)
local rentDE = getRentDE(Item)
    local newRent = function (dialog)
        ...
        if (string.find(input,"") ~= nil) then
            if tonumber(input) == nil then
                User:inform(getText(User,"Hier muss eine Zahl eingetragen werden.","Input must be a number."))
            elseif tonumber(input) <= 0 then
                User:inform(getText(User,
                "Die Zahl muss grer als 0 sein.","You must set a number higher than 0."))
            else
                ScriptVars:set("rentfor"..propertyName,input)
                User:inform(getText(User,"Mietpreis auf "..input.." gesetzt","Rent set to "..input))
                ScriptVars:save()
                setSignature(User,Item)
            end
        else
    ...
    User:requestInputDialog(InputDialog(getText(User,
    "Miete einstellen.","Set rent."),
     getText(User,"Aktuelle Miete: "..rentDE.."\nStelle eine neue monatliche Miete in Kupferstäcken ein.",
...
local propertyName = getPropertyName(Item)
local defaultRent = getDefaultRent(Item)
...
local foundRent, currentRent = ScriptVars:find("rentfor"..propertyName)
    if foundRent then
        ...
        local gCoins, sCoins, cCoins = money.MoneyToCoins(coins)
        if gCoins > 0 and sCoins == 0 and cCoins == 0 then
            return(gCoins.." Goldstäcke pro Monat.")
        elseif gCoins > 0 and sCoins > 0 and cCoins == 0 then
            return(gCoins.." Gold- und "..sCoins.." Silberstäcke pro Monat.")
        elseif gCoins > 0 and sCoins == 0 and cCoins > 0 then
            return(gCoins.." Gold- und "..cCoins.." Kupferstäcke pro Monat.")
        elseif gCoins == 0 and sCoins > 0 and cCoins == 0 then
            return(sCoins.." Silberstäcke pro Monat.")
        elseif gCoins == 0 and sCoins > 0 and cCoins > 0 then
            return(sCoins.." Silber- und "..cCoins.." Kupferstäcke pro Monat.")
        elseif gCoins == 0 and sCoins == 0 and cCoins > 0 then
            return(cCoins.." Kupferstäcke pro Monat.")
        elseif gCoins > 0 and sCoins > 0 and cCoins > 0 then
            return(gCoins.." Gold-, "..sCoins.." Silber- und "..cCoins.." Kupferstäcke pro Monat..")
        end
    else
        local coins = tonumber(defaultRent)
        local gCoins, sCoins, cCoins = money.MoneyToCoins(coins)
...
function setGuest(User, Item)
local propertyName = getPropertyName(Item)
local propertyNameDE = getPropertyNameDE(Item)
    ...
    local callback = function (dialog)
        ...
        local input = dialog:getInput()
            ...
            elseif foundGuest1 == false then
                ...
                ScriptVars:save()
                User:inform(getText(User,
                "Du hast "..input.."als Gast der "..propertyNameDE.." gesetzt. Schlässel die diese Person hat, \z
            ...
            elseif foundGuest2 == false then
                ...
                ScriptVars:save()
                User:inform(getText(User,
                "Du hast "..input.."als Gast der "..propertyNameDE.." gesetzt. Schlässel die diese Person \z
            ...
            elseif foundGuest5 == false then
                ...
                User:inform(getText(User,"Beide Gästeplätze sind belegt. Entferne einen bestehenden Gast von \z
                der Liste, um einen neuen eintragen zu können.",
                "Both guest spots are in use. Remove an \zexisting guest to add a new one."))
            end
    end
    User:requestInputDialog(InputDialog(getText(User,"Gästeliste","Guest list."),
     getText(User,"Gib den Namen des Gasts ein","Insert the name of the guest"), false, 255, callback))
end
-- Provides a list of current guests, allowing you to remove them to make room for more
function removeGuest(User, Item)
local propertyName = getPropertyName(Item)
local foundGuest1, currentGuest1 = ScriptVars:find("guest1"..propertyName)
local foundGuest2, currentGuest2 = ScriptVars:find("guest2"..propertyName)
local foundGuest3, currentGuest3 = ScriptVars:find("guest3"..propertyName)
...
local foundGuest5, currentGuest5 = ScriptVars:find("guest5"..propertyName)
    ...
    local dialog = SelectionDialog(getText(User,"Gästeliste","Guest List") ,
    getText(User,"Wähle einen Namen aus, der von der Gästeliste entfernt werden soll.",
    "Select a name to remove them from the guest list.") , callback)
    if foundGuest1 then
        dialog:addOption(0, getText(User,currentGuest1,currentGuest1))
    else
        dialog:addOption(0, getText(User,"Leerer Gästelisten-Platz","Vacant guest slot"))
    end
    if foundGuest2 then
        dialog:addOption(0, getText(User,currentGuest2,currentGuest2))
    else
        dialog:addOption(0, getText(User,"Leerer Gästelisten-Platz","Vacant guest slot"))
    end
    if foundGuest3 then
        dialog:addOption(0, getText(User,currentGuest3,currentGuest3))
    else
        dialog:addOption(0, getText(User,"Leerer Gästelisten-Platz","Vacant guest slot"))
    end
        if foundGuest4 then
        dialog:addOption(0, getText(User,currentGuest4,currentGuest4))
    else
        dialog:addOption(0, getText(User,"Leerer Gästelisten-Platz","Vacant guest slot"))
    end
        if foundGuest5 then
        dialog:addOption(0, getText(User,currentGuest5,currentGuest5))
    else
        dialog:addOption(0, getText(User,"Leerer Gästelisten-Platz","Vacant guest slot"))
    end
...
function checkIfOwnsProperty(Input)
     for i = 1, #M.propertyTable do
...
function checkIfPlayerIsGuest(User, propertyName)
local foundGuest1, currentGuest1 = ScriptVars:find("guest1"..propertyName)
local foundGuest2, currentGuest2 = ScriptVars:find("guest2"..propertyName)
local foundGuest3, currentGuest3 = ScriptVars:find("guest3"..propertyName)
local foundGuest4, currentGuest4 = ScriptVars:find("guest4"..propertyName)
...
function charOwnedDepotKeys(char)
    for i = 1, #M.depotList do
    local depNr = M.depotList[i]
    local depot = char:getDepot(depNr)
        if depot then
            local ownedDepotKeys = (depot:countItem(2558))+(depot:countItem(3054))+
            (depot:countItem(3055))+(depot:countItem(3056))
    ...
    local town = M.propertyTable[i][7]
        if not checkIfLeaderOfTown(char, town) then --Check if each property belongs to the specified town
            ...
            if foundOwner then --Check if there is an owner of the property
                -- Check if the character is the owner or guest of the property the key belongs to
                if not (char.name == currentOwner) and not checkIfPlayerIsGuest(char, propertyName) then
                    ...
                    -- Fetch how many keys character has in inventory
                    local keyAmount = char:countItemAt("all",keyType,{["lockId"]=keyID})
                    local depotKeyAmount = charOwnedDepotKeys(char) -- Fetch how many keys character has in depot
                ...
                local keyAmount = char:countItemAt("all",keyType,{["lockId"]=keyID})
                local depotKeyAmount = charOwnedDepotKeys(char)
                for y = 1, keyAmount do
                    debug("deleted key")
                    char:eraseItem(keyType,1,{["lockId"]=keyID})
                end
...
function M.keyRetrieval(char)
    ...
    if (charOwnedKeys(char) == 0) and (charOwnedDepotKeys(char) == nil) then
        return
    -- check if admin
...
local function getRequiredRankName(Item, language)
local property = getPropertyName(Item)
...
function getRankTitle(player, language)
...
local function setSignature(User,Item)
local property = getPropertyName(Item)
local rankTitleEN = getRankTitle(User, EN)
local rankTitleDE = getRankTitle(User, DE)
...
function getSignature(Item, language)
local property = getPropertyName(Item)
...
function getTownLeaderTitle(town, language)
...
function propertyInformation(User, Item)
local town = getTownName(Item)
local rent = getRent(Item)
local rentDE = getRentDE(Item)
local property = getPropertyName(Item)
local propertyDE = getPropertyNameDE(Item)
local rank = getRequiredRankName(Item, "EN")
local rankDE = getRequiredRankName(Item, "DE")
local remainingDuration = getRentDuration(Item)
local owner = checkOwner(Item)
local signatureEN = getSignature(Item, "EN")
local signatureDE = getSignature(Item, "DE")
local townLeaderTitle = getTownLeaderTitle(town, "EN")
local townLeaderTitleDE = getTownLeaderTitle(town, "DE")
    if checkOwner(Item) == "Unowned" then -- Shows info specific for when property is unowned
        local propertyInfo = MessageDialog(getText(User,"Notiz des Quartiermeisters","Quartermaster's notice"),
        getText(User,
        "Bärger von "..town..
        ",\nihr könnt nun die "..propertyDE..
        " zum Preis von "..rentDE..
        " mieten. Um dieses Grundstäck mieten zu können, mässt ihr zumindest "..rankDE..
        " sein. Solltest du dieses Grundstäck mieten wollen, wende dich an den Quartiermeister oder melde \z
    ...
    elseif checkOwner(Item) == User.name then -- Shows info specific for when property is owned by user
        local propertyInfo = MessageDialog(getText(User,"Notiz des Quartiermeisters","Quartermaster's notice"),
        getText(User,
        ...
        " Monaten aus.\nFär weitere Fragen oder Anmerkungen, wende dich an den Quartiermeister oder melde dich \z
        ...
        getText(User,
        "Bärger von "..town..
        ",\n dieses Grundstäck ist aktuell gemietet von "..owner..
        ". Solltest du irgendwelche Bedenken haben oder ein freies Grundstäck mieten wollen, wende dich bitte an \z
...
function abandonPropertyDialog(User, Item)
    local callback = function(dialog)
        local success = dialog:getSuccess()
        if success then
            local selected = dialog:getSelectedIndex()+1
            if selected == 1 then
                abandonProperty(User, Item)
                User:inform(getText(User,"Nachdem du dem Quartiermeister den Schlässel zuräckgegeben hast, \z
                verabschiedest du dich von deinem alten Heim. Wo wirst du nun schlafen?",
                "Having returned the key to the Quartermaster, you bid farewell to your old home. \z
                Now where will you sleep?"))
            else
                User:inform(getText(User,"Du hast dich entschieden das Mietverhältnis nicht zu beenden. ",
                "You decide against giving up on your lease for the property."))
            end
        end
    end
    local dialog = SelectionDialog(getText(User,"Mietverhältnis beenden","Abandon property"),getText(User,"",
    "Are you sure you want to abandon your lease on the property? Any remaining rent money will not be returned."),
    callback)
    dialog:addOption(0, getText(User,"Ja, Mietverhältnis beenden","Yes, abandon it"))
    dialog:addOption(0, getText(User,"Nein, ich habe meine Meinung geändert","No, I changed my mind"))
    User:requestSelectionDialog(dialog)
end
function abandonProperty(User, Item)
    local propertyName = getPropertyName(Item)
    ScriptVars:remove("ownerof"..propertyName)
    ScriptVars:remove("guest1"..propertyName)
    ScriptVars:remove("guest2"..propertyName)
...
local foundCurrentRentDuration, currentRentDuration = ScriptVars:find("rentDuration"..propertyName)
    ...
    local dialog = function (dialog)
        ...
        if (string.find(input,"") ~= nil) then
            if tonumber(input) == nil then
                User:inform(getText(User,
                "Hier muss eine Zahl eingetragen werden.",
                "Input must be a number."))
            elseif tonumber(input) <= 0 then
                User:inform(getText(User,
                "Die Zahl muss grer als 0 sein.",
                "You must set a number higher than 0."))
            else
                ScriptVars:set("rentDuration"..propertyName,(currentRentDuration+input))
                User:inform(getText(User,
                "Mietdauer verlängert um"..input..
                " Monate","Rent duration extended by "..input..
                " months"))
                ScriptVars:save()
            end
        else
    ...
    User:requestInputDialog(InputDialog(getText(User,"Miete verlängern","Extend rent"), getText(User,
    "Mietdauer fär den aktuellen Mieter umsonst verlängern.\n Derzeit beträgt \z
    die Mietdauer noch "..currentRentDuration..
    " Monate.","Extend rent for current renter at no charge.\n There's currently "..currentRentDuration..
    " months left on the lease."), false, 255, dialog))
end
    ...
    for i = 1, #M.propertyTable do
        ...
        if foundOwner then
            if currentOwner == User.name then
                ...
                ScriptVars:save()
                User:inform(getText(User,
                "Nachdem du nicht länger Bärger von "..town..
                " bist, hast du das Recht auf deinen Mietanspruch auf die "..propertyNameDE..
                " verloren und wirst gezwungen die Schlässel zuräckzugeben.",
                "Because you are no longer a citizen of "..town..
                " you have forfeited the lease on the "..propertyName..
                " and been made to return your keys."))
            end
        end
    end
end
...
function M.informUserOfKeyRetrieval(User)
    for i = 1, #M.propertyTable do
        ...
        if foundOwner then
            if currentOwner ~= User.name then
                if checkIfLeaderOfTown(User, town) or User:isAdmin() then
                    return
                else
                local keyID = M.propertyTable[i][6]
                local keyType = M.propertyTable[i][5]
                local keyAmount = User:countItemAt("all",keyType,{["lockId"]=keyID})
                local depotKeyAmount = charOwnedDepotKeys(User)
                    if tonumber(keyAmount) > 0 then
                        User:inform(getText(User,"Bei deinem letzten Besuch in "..town.." wurden bei einer \z
                        routinemigen Taschenkontrolle durch die Wachen Schlässel, die du nicht \z
                        besitzen solltest, gefunden und konfisziert.",
                        "Upon your latest visit to "..town.." some keys you were not supposed to have were \z
                        confiscated by the guards in a random bag check at the gates."))
                        return
        ...
        elseif checkIfLeaderOfTown(User, town) or User:isAdmin() then
            return
        else
        local keyID = M.propertyTable[i][6]
        local keyType = M.propertyTable[i][5]
        ...
        local depotKeyAmount = charOwnedDepotKeys(User)
            if tonumber(keyAmount) > 0 then
                User:inform(getText(User,"Bei deinem letzten Besuch in "..town.." wurden bei einer \z
                routinemigen Taschenkontrolle durch die Wachen Schlässel, die du nicht besitzen solltest, \z
                gefunden und konfisziert.","Upon your latest visit to "..town.." some keys you were not supposed \z
                to have were confiscated by the guards in a random bag check at the gates."))
                return
            elseif depotKeyAmount ~= nil then
                if tonumber(depotKeyAmount) > 0 then
                    User:inform(getText(User,
                    "Bei deinem letzten Besuch in "..town.." wurden bei einer routinemigen Taschenkontrolle \z
                    durch die Wachen Schlässel, die du nicht besitzen solltest, gefunden und konfisziert.",
                    "Upon your latest visit to "..town.." some keys you were not supposed to have were confiscated \z
                    by the guards in a random bag check at the gates."))
                    return
                end
            end
        end
...
function M.LookAtItem(User,Item)
    ...
    for i = 1, #M.propertyTable do
        if (Item.pos == M.propertyTable[i][3]) then
            lookAt = propertyDeedLookAt(User, Item)
        end
    end
        ...
        if (SourceItem.pos == M.propertyTable[i][3]) then
            ...
            local callback1 = function(dialogUnowned)
                ...
                if success then
                    ...
                    if selected == 1 then
                        propertyInformation(User, SourceItem)
                    elseif selected == 2 then
                        setOwner(User, SourceItem)
                    elseif selected == 3 then
                        setRent(User, SourceItem)
                    else
                        setReqRank(User, SourceItem)
                    end
                end
            end
            local callback2 = function(dialogOwnedByUser)
                ...
                if success then
                    ...
                    if selected == 1 then
                        propertyInformation(User, SourceItem)
                    elseif selected == 2 then
                        abandonPropertyDialog(User, SourceItem)
                    elseif selected == 3 then
                        setGuest(User, SourceItem)
                    elseif selected == 4 then
                        removeGuest(User, SourceItem)
                    elseif selected == 5 then
                        setOwner(User, SourceItem)
                    elseif selected == 6 then
                        removeOwner(User, SourceItem)
                    elseif selected == 7 then
                        setRent(User, SourceItem)
                    elseif selected == 8 then
                        extendRent(User, SourceItem)
                    else
                        setReqRank(User, SourceItem)
                    end
                end
            ...
            local dialogUnowned = SelectionDialog(getText(User,getPropertyNameDE(SourceItem),
            getPropertyName(SourceItem)),
             getText(User,"Wähle aus, was du machen willst.","Select what you want to do.") , callback1)
            local dialogOwnedByUser = SelectionDialog(getText(User,getPropertyNameDE(SourceItem),
            getPropertyName(SourceItem)),
             getText(User,"Wähle aus, was du machen willst.","Select what you want to do.") , callback2)
            local dialogOwnedNotByUser = SelectionDialog(getText(User,getPropertyNameDE(SourceItem),
            getPropertyName(SourceItem)),
             getText(User,"Wähle aus, was du machen willst.","Select what you want to do.") , callback3)
            dialogUnowned:addOption(0, getText(User,"Betrachte 'Notiz des Quartiermeisters'",
            "Inspect 'Notice from the Quartermaster'"))
            dialogOwnedByUser:addOption(0, getText(User,"Betrachte 'Notiz des Quartiermeisters'",
            "Inspect 'Notice from the Quartermaster'"))
            dialogOwnedByUser:addOption(0, getText(User,"Mietverhältnis beenden","Abandon Property"))
            dialogOwnedNotByUser:addOption(0, getText(User,"Betrachte 'Notiz des Quartiermeisters'",
            "Inspect 'Notice from the Quartermaster'"))
            if checkIfLeaderOfTown(User, town) or User:isAdmin() then
                dialogUnowned:addOption(0, getText(User,"Mieter eintragen","Set Renter"))
                dialogUnowned:addOption(0, getText(User,"Miete anpassen","Change Rent"))
                dialogUnowned:addOption(0, getText(User,"Miete anpassen","Change Required Rank"))
                dialogOwnedByUser:addOption(0, getText(User,"Gast hinzufägen","Add Guest"))
                dialogOwnedByUser:addOption(0, getText(User,"Gast entfernen","Remove Guest"))
                dialogOwnedByUser:addOption(0, getText(User,"Mieter eintragen","Set Renter"))
                dialogOwnedByUser:addOption(0, getText(User,"Mieter entfernen","Remove Renter"))
                dialogOwnedByUser:addOption(0, getText(User,"Miete anpassen","Change Rent"))
                dialogOwnedByUser:addOption(0, getText(User,"Miete verlängern","Extend Rent"))
                dialogOwnedByUser:addOption(0, getText(User,"Miete anpassen","Change Required Rank"))
                dialogOwnedNotByUser:addOption(0, getText(User,"Gast hinzufägen","Add Guest"))
                dialogOwnedNotByUser:addOption(0, getText(User,"Gast entfernen","Remove Guest"))
                dialogOwnedNotByUser:addOption(0, getText(User,"Mieter eintragen","Set Renter"))
                dialogOwnedNotByUser:addOption(0, getText(User,"Mieter entfernen","Remove Renter"))
                dialogOwnedNotByUser:addOption(0, getText(User,"Miete anpassen","Change Rent"))
                dialogOwnedNotByUser:addOption(0, getText(User,"Miete verlängern","Extend Rent"))
                dialogOwnedNotByUser:addOption(0, getText(User,"Miete anpassen","Change Required Rank"))
            end
            if checkOwner(SourceItem) == "Unowned" then
                User:requestSelectionDialog(dialogUnowned)
            elseif checkOwner(SourceItem) == User.name then
