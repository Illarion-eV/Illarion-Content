...
function M.UseItem(User, SourceItem, ltstate)
    ...
    if User:getQuestProgress(325) == 1 and User:getQuestProgress(340) == 0 then --Accepted tutorial messages, didn't get the message before
        ...
        local callbackNewbie = function(informNewbie)
            User:inform("WÃ¤hrend du die Zutaten in dem riesigen Fass umrürst, erfüllt ein Hauch gärender Äpfel deine Nase.","As you stir ingredients in the immense barrel a waft of fermenting apples fills your nostrils.")
            brewing.brewing:showDialog(User, SourceItem)
        end --end callback
        local dialogText = common.GetNLS(User,
            "Da du erst anfangen hast, wirst du wahrscheinlich nur leicht alkoholhaltige GetrÃ¤nke auf deiner Braukarte haben. Öffne die Kategorie 'Leichter Alkohol', indem du auf den vorangestellten Pfeil klickst und auf 'Ciderflasche' klickst, um sie auszuwählen. In diesem Fall wird nur eine Flasche Apfelcider benötigt, aber du kannst die Anzahl der zu produzierenden Artikel mit den Schaltflächen '-' und '+' anpassen. Starte die Produktion, indem du auf die Schaltfläche 'Herstellen' klickst. Ein Fortschrittsbalken zeigt an, wie lange es bis zum Abschluss der Produktion dauert, und deine fertige Flasche Apfelcider erscheint nach Beendigung in deiner Tasche.",
