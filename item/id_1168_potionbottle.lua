...
-- UPDATE items SET itm_script='item.desk' WHERE itm_id IN (1168);
...
function M.UseItem(User, SourceItem)
    ...
    elseif SourceItem.pos == position(607, 177, -3) and (User:getQuestProgress(533) == 1) then -- player adding red potion to the yellow
    ...
    elseif SourceItem.pos == position(607, 177, -3) and (User:getQuestProgress(536) == 1) and (User:getQuestProgress(541) > 0) then -- player adding green potion to the yellow but already has a heath buff
        User:inform(
    ...
    elseif SourceItem.pos == position(607, 177, -3) and (User:getQuestProgress(536) == 1) then -- player adding green potion to the yellow
    ...
    elseif SourceItem.pos == position(607, 177, -3) and (User:getQuestProgress(535) == 1) then -- player adding violet potion to the yellow
    ...
    elseif SourceItem.pos == position(607, 177, -3) and (User:getQuestProgress(538) == 1) then -- player adding lt green potion to the yellow
        world:gfx(44, User.pos) -- explosion gfx
        world:makeSound(5, User.pos) --a loud boom
        User:inform(
            "Die Mischung dieser Tränke verursacht eine Explosion. Du fühlst, wie sich deine Gesundheit verschlechtert.",
    ...
    elseif SourceItem.pos == position(607, 177, -3) and (User:getQuestProgress(537) == 1) and (User:getQuestProgress(540) == 1) then -- player adding dark blue potion to the yellow and has already got the special potion
        User:inform(
            "",
            "Although you have had previous success with this combination, the luck of Nargun is no longer with you.")
    elseif SourceItem.pos == position(607, 177, -3) and (User:getQuestProgress(537) == 1) then -- player adding dark blue potion to the yellow
        local notCreated = User:createItem(329, 1, 222, {["potionEffectId"] = 521}) -- halfing transformation potion
