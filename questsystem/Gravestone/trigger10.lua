require("handler.sendmessagetoplayer")
require("handler.createeffect")
require("questsystem.base")
module("questsystem.Gravestone.trigger10", package.seeall)

local QUEST_NUMBER = 10000
local PRECONDITION_QUESTSTATE = 76
local POSTCONDITION_QUESTSTATE = 58

local POSITION = position(605, 344, 0)
local RADIUS = 1

function UseItem( PLAYER, item, TargetItem, counter, Param, ltstate )
  if PLAYER:isInRangeToPosition(POSITION,RADIUS)
      and ADDITIONALCONDITIONS(PLAYER)
      and questsystem.base.fulfilsPrecondition(PLAYER, QUEST_NUMBER, PRECONDITION_QUESTSTATE) then
    --informNLS(PLAYER, TEXT_DE, TEXT_EN)
    
    HANDLER(PLAYER)
    
    questsystem.base.setPostcondition(PLAYER, QUEST_NUMBER, POSTCONDITION_QUESTSTATE)
    return true
  end

  return false
end

function informNLS(player, textDe, textEn)
  if player:getPlayerLanguage() == Player.german then
    player:inform(player, item, textDe)
  else
    player:inform(player, item, textEn)
  end
end

-- local TEXT_DE = TEXT -- German Use Text -- Deutscher Text beim Benutzen
-- local TEXT_EN = TEXT -- English Use Text -- Englischer Text beim Benutzen


function HANDLER(PLAYER)
    handler.sendmessagetoplayer.sendMessageToPlayer(PLAYER, "Die Firnisblüte verschwindet vor deinen Augen.", "The firnis blossom disappears in front of your eyes."):execute()
    handler.createeffect.createEffect(position(605, 344, 0), 32):execute()
    handler.sendmessagetoplayer.sendMessageToPlayer(PLAYER, "Du erschrickst kurz als die Firnisblüte verschwindet.", "You get scared a bit as you notice that the firnis blossom disappears."):execute()
end

function ADDITIONALCONDITIONS(PLAYER)
return true
end