...
-- Full Mana and food, + 10% health, fairy shower
    ...
    common.InformNLS(targetChar, "Du f黨lst dich frisch und satt.", "You feel fresh and well-feed.")
...
-- lightspot, wind
    ...
    common.InformNLS(targetChar, "Ein sanfter Wind umspielt deinen Kopf. Du f黨lst dich gut.", "A soft wind touches your head. You feel good.")
...
-- Take 25% Mana and food, - 10% health, lightning, thunder
    ...
    world:makeSound(globalvar.sfxTHUNDER,targetChar.pos)
    common.InformNLS(targetChar, "Bilder zorniger G鰐ter laufen vor deinen Augen ab.", "Visions of angry Gods fill your head.")
...
local function settingsForCharMagicClass(User, chosenPlayer)
    local magicClassDialog = function (dialog)
        ...
        if targetClass ~= chosenPlayer:getMagicType() then
            ...
            common.InformNLS(chosenPlayer, "[GM Info] Die magische Klasse wurde auf " .. classNames[targetClass] .. "ge鋘dert", "[GM Info] The magic class has been changed to" .. classNames[targetClass].. ".")
        end
    end
...
local function settingsForCharAttacableLimited(User, chosenPlayer)
    if chosenPlayer:getQuestProgress(236) == 0 then
        ...
        common.InformNLS(chosenPlayer, "[GM Info] Dein Char wird f黵 ca. 15 Minuten nicht von NPC angegriffen.", "[GM Info] Your character is unable to be attacked by NPC for about 15 min.")
    else
...
local function settingsForCharFireProof(User, chosenPlayer)
    if chosenPlayer:getQuestProgress(298) == 0 then
        ...
        common.InformNLS(chosenPlayer, "[GM Info] Dein Char wird f黵 ca. 15 Minuten gegen magische Flammen resistent sein.", "[GM Info] Your character is proof against magic flames for about 15 min.")
    else
...
local function settingsForCharIceFlameProof(User, chosenPlayer)
    if chosenPlayer:getQuestProgress(299) == 0 then
        ...
        common.InformNLS(chosenPlayer, "[GM Info] Dein Char wird f黵 ca. 15 Minuten gegen Eisflammen resistent sein.", "[GM Info] Your character is proof against ice flames for about 15 min.")
    else
...
local function settingsForCharPoisonCloudProof(User, chosenPlayer)
    if chosenPlayer:getQuestProgress(300) == 0 then
        ...
        common.InformNLS(chosenPlayer, "[GM Info] Dein Char wird f黵 ca. 15 Minuten gegen Giftwolken resistent sein.", "[GM Info] Your character is proof against poison clouds for about 15 min.")
    else
