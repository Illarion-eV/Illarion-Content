...
function GetCow(message, originator)
        ...
        if (originator:getQuestProgress(8) == 0) then
            ...
            if CheckMoney(originator,GCoins,SCoins,CCoins) then
                ...
                if not posOfCow then
                    ...
                    thisNPC:talk(Character.say, "Hier habt ihr den Esel. Bringt sie sicher wieder dann bekommt ihr die "..(Kaution/100).." Silberstücke Kaution wieder.", "There you have your mule. Bring her back safely then you get your "..(Kaution/100).." silvercoins surety back.")
    ...
    or (string.find(message,"buy.+donkey"   )~=nil) then
        if (string.find(message,"[Ee]sel")~=nil) or (string.find(message,"mule")~=nil) then
            thisNPC:talk(Character.say, "Ich verkaufe keine Esel. Ich verleihe sie höchstens." , "I don't sell mules. I just borrow them." )
        else
            thisNPC:talk(Character.say, "Ich verkaufe keine Lasttiere. Ich verleihe sie höchstens." , "I don't sell pack animals. I just borrow them." )
        end
    end
    return false
end
