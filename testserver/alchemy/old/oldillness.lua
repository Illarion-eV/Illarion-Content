...
-- OLD !!!
-- old illness and medicine 
-- maybe some stuff can be used postVBU
...
function DrinkPotion(Character,SourceItem)
	...
	local dataZList = druid.base.alchemy.SplitBottleData(Character,SourceItemREPLACED);
	...
	if not found then
		...
		if SourceItemREPLACED == 83795161 then
			--     Character:inform("Ork-Fieber/orc-fever")
			diagnose=1
		elseif SourceItemREPLACED == 26343194 then
			--     Character:inform("Sumpfkrampf/bog-attack")
			diagnose=2
		elseif SourceItemREPLACED == 98886573 then
			--     Character:inform("Trollsucht/Troll's rash")
			diagnose=3
		elseif SourceItemREPLACED == 65336351 then
			--     Character:inform("Gnom-Wahn/gnome-paranoia")
			diagnose=4
		elseif math.mod(SourceItemREPLACED,10000000) == 8455363 then
			--     Character:inform("Vein'sches Syndrom/Veins'syndrome")
			diagnose=5
			local allergy = math.floor(SourceItemREPLACED/10000000);
			if allergy == 3 then
				allergy = 1;
		...
		elseif SourceItemREPLACED == 95819741 then
			--     Character:inform("Drachenpocken/dragon's pox")
			diagnose=6
		elseif SourceItemREPLACED == 15386558 then
			--     Character:inform("Skorpion-Seuche/scorpion's pestilence")
			diagnose=7
		elseif SourceItemREPLACED == 58595367 then
...
function PoisonCharacter(User,SourceItem,Character)
	...
	if base.common.IsLookingAt(User,Character.pos) then
		if not base.common.IsLookingAt(Character,User.pos) then
			local SkillName=Character.poisoning;
...
--
function UseItem(Character,SourceItem,TargetItem,ltstate)
