...
module("druids.items.167_yellow_bottle", package.seeall(druids.base.alchemy))

-- UPDATE common SET com_script='druids.items.167_yellow_bottle' WHERE com_itemid = 167;
            ...
            if base.common.IsLookingAt(User,Character.pos) then
                if not base.common.IsLookingAt(Character,User.pos) then
                    ...
                    if (PoiTry>PoiDef) then
                        ...
                        base.common.InformNLS(Character,"Jemand hat dir Gift in den in den Mund geschüttet.","Someone pour you a toxin into your mouth.");
                        world:erase(SourceItem,1);
                        world:makeSound(12,Character.pos);
				Character:setPoisonValue( base.common.Limit( (Character:getPoisonValue() + (math.floor((825/100)*(SkillVal-AttribValDef)+(175/10)))) , 0, 10000) );
                        ...
                        base.common.InformNLS(Character,"Jemand versuchte dir den Inhalt einer Flasche in den Mund zu schütten.","Someone tried to make you drink a potion.");
                        world:erase(SourceItem,1);
                    end
...
function LookAtItem(User,Item)
  ...
  elseif math.mod(Item.data,10000000) == 8455363 then
