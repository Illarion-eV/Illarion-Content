...
--        Informing(Defender,gText,eText);
        ...
        if not AusweichWurf then
            ...
            if (AngriffsWurf>ParadeWurf) then
                ...
                -- hier modifizieren
                FeuerWurf=FireTry(Attacker);
                if (FeuerWurf>0) then
                    world:gfx(37,Defender.pos)
                end
                ...
                DmgTaken=math.floor(SchadensWurf*(100-AbwehrWurfPer)/100)-AbwehrWurfHP+FeuerWurf;
...
--                    eGiftText="";
--                end    
--                gText=Attacker.name.." trifft "..Defender.name.." in Trefferzone \""..AreaNames[1].."\" mit "..SchadensWurf.." HP. "..Defender.name.." absorbiert "..AbwehrWurfPer.." % und "..AbwehrWurfHP.." HPs. Verursachter Schaden: "..math.max(0,DmgTaken).."HP. Davon Feuerschaden: "..FeuerWurf.." "..gGiftText;
--                eText=Attacker.name.." hits "..Defender.name.." in Hitarea \""..AreaNames[2].."\" with "..SchadensWurf.." HP. "..Defender.name.." absorbs "..AbwehrWurfPer.."% and "..AbwehrWurfHP.." HP. Produced damage: "..math.max(0,DmgTaken).."HP. Therefrom Firedamage: "..FeuerWurf.." "..eGiftText;
--                Informing(Attacker,gText,eText);
--                Informing(Defender,gText,eText);
...
function FireTry(Attacker)
    local FireHit=0;
    local RndFire=math.random(1,6)
    if (RndFire==1) then
        FireHit=Scale(200,1000,AttSkill);
        FireHit=FireHit*(math.random(80,120)/100)
    end
    return FireHit
end