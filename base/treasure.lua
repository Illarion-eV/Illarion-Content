...
local M = {}
    ...
    function M.GetTreasureName( level, lang, details )
        if details then
            ...
            if ( not level ) or ( level == 0) then
                return ( lang == 0 and "Schatzkiste" or "treasure chest" );
            elseif ( level <= 2 ) then
    ...
    function GetMonsterList( level )
        ...
        monsters[-1]={271,274,278,581,582,592,601,622,982,1031,1051,1071}; --Level 1
        monsters[0]=monsters[-1]; --Level 1
        monsters[1]=monsters[-1]; --Level 1
        monsters[2]={101,103,104,107,114,151,172,196,272,273,392,393,583,593,602,762,881,891,573,577,953,955,983,606,
            1032,1041,1052}; --Level 2
        monsters[3]={21,24,102,111,112,115,152,171,311,391,394,403,404,516,551,571,594,603,611,612,752,753,763,536,871,
            882,892,574,575,578,951,952,954,981,984,605,1033,1042,1053}; --Level 3
        monsters[4]={5,14,22,23,25,43,55,91,113,141,173,211,301,395,401,515,512,521,552,553,572,604,613,105,754,761,
            764,791,535,539,872,883,893,901,576,1043}; --Level 4
        monsters[5]={1,3,4,11,13,15,31,33,34,35,41,44,45,51,53,54,92,93,94,142,191,192,321,402,514,511,522,523,531,532,
            614,108,741,751,782,783,784,792,801,537,540,851,861,873,902}; --Level 5
        monsters[6]={2,12,32,42,52,61,62,63,64,65,95,121,122,143,193,201,202,221,231,331,513,524,525,533,541,561,109,
            742,744,802,811,578,852,862,903}; --Level 6
        monsters[7]={123,124,131,195,203,204,232,261,341,534,542,543,562,633,635,636,110,743,796,853,863}; --Level 7
        monsters[8]={125,132,205,262,631,634,637,564}; --Level 8
        monsters[9]={302,632,563}; --Level 9
    ...
    function M.getDistance( User, Item )
        ...
        elseif ( metricDistance < 500 ) then
            ...
            return common.GetNLS( User, "‰uﬂerst fern", "extremely far" );
        end
    end
