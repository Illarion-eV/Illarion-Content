...
function M.MoveToField(user)
    ...
    if getCharacterLoad(user) >= getMaximumLoad(user)/5 then
        ...
        if user:getRace() == 5 then
            User:inform("Durch das Gewicht deiner Ausrüstung, wirst du tief ins Wasser gezogen und von einer Strömung erfasst, doch durch das natürliche Schwimmvermögen deiner Rasse kannst du dich ihr entgegenstemmen.","")
        else
            user:inform("Durch das Gewicht deriner Ausrüstung, wirst du tief ins Wasser gezogen und von einer Strömung erfasst!", "")
            user:warp(2, 22, 0)
            user:talk(Character.say, "#me wird ans Ufer gespühlt.", "")
        end
