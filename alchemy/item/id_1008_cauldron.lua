  ...
  -- prüfe ob der User eine Planze in der Hand hat
    ...
    if plantInHand then
		...
		if bottleInHand.id == 331 then
		   if math.random(1,100) <= math.floor(120/(User:getSkill(Skill.alchemy)+User:increaseAttrib("perception",0))) then
		      quali = bottleInHand.quality - 100;
          else 
		      quali = bottleInHand.quality;
		   end		
		elseif bottleInHand.id == 164 then
		       quali = math.floor(User:getSkill(Skill.alchemy)+User:increaseAttrib("perception",0)*8);
		end
