...
module("druid.item.id_327_blue_bottle", package.seeall)
...
function MoveItemBeforeMove( User, SourceItem, TargetItem )
    ...
    if (SourceItem:getType()~=4 or (Sourceitem.id_itempos~=5 and Sourceitem.id_itempos~=6)) then
        ...
        "Du musst den Wurfkï¿½rper aus der Hand werfen.",
...
function UseItem(User,SourceItem,TargetItem,counter,param)
	if Sourceitem.id_data == 0 then
		...
		elseif checkMissile(Sourceitem.id_data) then --das ist ein Wurfkï¿½rper
			if (math.floor(Sourceitem.id_quality/1000)==1) then -- Wurfkï¿½rper gesichert (qual: 1xxx) --> entsichern (qual: xxx)
            	base.common.TempInformNLS( User,
            	"Du entsicherst des Wurfkï¿½rper. Vorsicht damit.",
...
function LookAtItem(Character,Item)
	...
	elseif (item.id_data == 0) then
		...
		if (Character:getPlayerLanguage()==0) then
			world:itemInform(Character,Item,"Du siehst ein Flaschenetikett mit der Aufschrift: 'Wurfkï¿½rper'")
		else
