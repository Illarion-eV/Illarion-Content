...
function onSpawn(Monster)
	...
	if MonID == 681 then --Shiny Deathdragon
...
function FireBreath(Monster,Enemy)
    ...
    if (BreathTry==1) and (Monster.pos.z==Enemy.pos.z) then
        ...
        if (Monster:distanceMetric(Enemy)<=4) then
            ...
            monster.base.drop.CastMonMagic(Monster,Enemy,1,{2500,2500},{{6,5}},{},0,1,{50,100})
        end
    end
...
function onDeath(Monster)
    ...
    if (MonID==681) then --Shiny Deathdragon, Level: 7, Armourtype: heavy, Weapontype: slashing
        ...
        if not done then done=monster.base.drop.AddDropItem(446,1,1,(100*math.random(6,7)+math.random(66,77)),0,1); end --sapphire powder
