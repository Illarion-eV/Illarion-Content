...
function M.enemyNear(Monster,Enemy)
	...
	if (MonID==892) then
        return ( drop.CastMonMagic(Monster,Enemy,7,{250,750},{{36,5},{9,5},{38,5}},{},40,1,{35,50}) );
	elseif (MonID==893) then
        return ( monstermagic.CastMonster(Monster, {622, 1032}) );
    else
        return false;
    end
end
...
if (MonID==891) then --Fireimp, Level: 2, Armourtype: light, Weapontype: puncture
        ...
        local done=drop.AddDropItem(2760,1,20,(100*math.random(1,2)+math.random(11,22)),0,1); --rope
        if not done then done=drop.AddDropItem(155,1,10,(100*math.random(1,2)+math.random(11,22)),0,1); end --sibanac
        if not done then done=drop.AddDropItem(55,1,1,(100*math.random(1,2)+math.random(11,22)),0,1); end --green robe
        if not done then done=drop.AddDropItem(817,1,1,(100*math.random(1,2)+math.random(11,22)),0,1); end --green tunic
        if not done then done=drop.AddDropItem(194,1,1,(100*math.random(1,2)+math.random(11,22)),0,1); end --black robe

        --Category 2: Special Loot

        local done=drop.AddDropItem(197,1,20,(100*math.random(1,2)+math.random(11,22)),0,2); --amethyst
        if not done then done=drop.AddDropItem(46,1,10,(100*math.random(1,2)+math.random(11,22)),0,2); end --ruby
        if not done then done=drop.AddDropItem(733,1,1,(100*math.random(1,2)+math.random(11,22)),0,2); end --stone block
        if not done then done=drop.AddDropItem(735,1,1,(100*math.random(1,2)+math.random(11,22)),0,2); end --raw stone
        if not done then done=drop.AddDropItem(1266,20,1,(100*math.random(1,2)+math.random(11,22)),0,2); end --stone
        ...
        if not done then done=drop.AddDropItem(189,1,1,(100*math.random(1,2)+math.random(11,22)),0,3); end --dagger
        if not done then done=drop.AddDropItem(190,1,1,(100*math.random(1,2)+math.random(11,22)),0,3); end --simple dagger
        if not done then done=drop.AddDropItem(2740,1,1,(100*math.random(1,2)+math.random(11,22)),0,3); end --red dagger
    ...
    elseif (MonID==892) then --Cures Fireimp, Level: 3, Armourtype: light, Weapontype: slashing
        ...
        if not done then done=drop.AddDropItem(365,1,1,(100*math.random(2,3)+math.random(22,33)),0,1); end --half leather armor
        if not done then done=drop.AddDropItem(53,1,1,(100*math.random(2,3)+math.random(22,33)),0,1); end --leather boots
        ...
        local done=drop.AddDropItem(64,10,20,(100*math.random(2,3)+math.random(22,33)),0,3); --arrows
        if not done then done=drop.AddDropItem(237,5,10,(100*math.random(2,3)+math.random(22,33)),0,3); end --bolts
        if not done then done=drop.AddDropItem(56,1,1,(100*math.random(2,3)+math.random(22,33)),0,3); end --bough
        if not done then done=drop.AddDropItem(826,1,1,(100*math.random(2,3)+math.random(22,33)),0,3); end --black trousers
