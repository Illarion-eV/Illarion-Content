...
function enemyNear(Monster,Enemy)
    ...
    if (MonID==202) then
        return ( drop.CastMonMagic(Monster,Enemy,5,{1500,3500},{{36,5},{9,5}},{},40,1,{25,65}) );
	elseif (MonID==204) then
        return ( drop.CastMonMagic(Monster,Enemy,5,{2000,4000},{{3,5},{6,5}},{},40,1,{25,65}) );
    elseif (MonID==205) then
        return ( monstermagic.SuddenWarp(Monster, Enemy) or monstermagic.CastMonster(Monster, {103, 763}) );
    else
        return false;
    end
end
...
function enemyOnSight(Monster,Enemy)
	...
	elseif (MonID==204) then
        return ( drop.CastMonMagic(Monster,Enemy,5,{2000,4000},{{3,5},{6,5}},{},40,1,{25,65}) );
    elseif (MonID==205) then
        return ( monstermagic.CastMonster(Monster, {103, 763}) );
    else
        return false;
    end
end
...
if (MonID==201) then --Demon Skeleton, Level: 6, Armourtype: medium, Weapontype: slashing
    ...
    elseif (MonID==202) then --Mystic Demon Skeleton, Level: 6, Armourtype: light, Weapontype: concussion (wrestling)
        ...
        local done=drop.AddDropItem(357,1,20,(100*math.random(5,6)+math.random(55,66)),0,1); --blue wizard hat
        if not done then done=drop.AddDropItem(371,1,10,(100*math.random(5,6)+math.random(55,66)),0,1); end --expensive wizard hat
        if not done then done=drop.AddDropItem(547,1,1,(100*math.random(5,6)+math.random(55,66)),0,1); end --novice mage robe
        if not done then done=drop.AddDropItem(548,1,1,(100*math.random(5,6)+math.random(55,66)),0,1); end --mage robe
        if not done then done=drop.AddDropItem(558,1,1,(100*math.random(5,6)+math.random(55,66)),0,1); end --master mage robe

        --Category 2: Special loot

        local done=drop.AddDropItem(68,1,20,(100*math.random(5,6)+math.random(55,66)),0,2); --ruby ring
        if not done then done=drop.AddDropItem(278,1,10,(100*math.random(5,6)+math.random(55,66)),0,2); end --obsidian ring
        if not done then done=drop.AddDropItem(336,1,1,(100*math.random(5,6)+math.random(55,66)),0,2); end --mirror
        if not done then done=drop.AddDropItem(334,1,1,(100*math.random(5,6)+math.random(55,66)),0,2); end --charm of the icebird
        if not done then done=drop.AddDropItem(46,1,1,(100*math.random(5,6)+math.random(55,66)),0,2); end --ruby

        --Category 3: Weapon

        local done=drop.AddDropItem(76,1,20,(100*math.random(5,6)+math.random(55,66)),0,3); --mage's staff
        if not done then done=drop.AddDropItem(1044,1,10,(100*math.random(5,6)+math.random(55,66)),0,3); end --ornate mage's staff
        if not done then done=drop.AddDropItem(323,1,1,(100*math.random(5,6)+math.random(55,66)),0,3); end --wand
    ...
    elseif (MonID==203) then --Unholy Akolyth, Level: 7, Armourtype: heavy, Weapontype: concussion
        ...
        if not done then done=drop.AddDropItem(184,1,1,(100*math.random(6,7)+math.random(66,77)),0,1); end --black visor helmet
    ...
    elseif (MonID==204) then --Unholy Archmage, Level: 7, Armourtype: heavy, Weapontype: slashing
        ...
        if not done then done=drop.AddDropItem(278,1,10,(100*math.random(6,7)+math.random(66,77)),0,2); end --obsidian ring
        ...
        if not done then done=drop.AddDropItem(209,1,1,(100*math.random(6,7)+math.random(66,77)),0,3); end --battle staff
