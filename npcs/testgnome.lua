...
function initializeNpc()
    ...
    npcs.base.functions.AddCycleText("#me scheppert mit einem Ledersäckchen","#me clangs with a small leather bag"); -- Fügt Text der alle paar Min gesagt wird hinzu
    ...
    if npcStatus[user.id]==3 then    -- npc muss ziehen
        ...
        repeat
            ...
            if stoneSumNPC>11 then 
                finished=true;
                npcStatus[user.id]=0;
                sayText=npcs.base.functions.GetNLS(user,"Oh, ich habe verloren. Hier, dein Gewinn, 10 Kupferstücke.","Oh, I lost. Here's your reward: 10 copper coins.");
                thisNPC:talk(CCharacter.say, sayText);
    ...
    if npcs.base.functions.BasicNPCChecks(originator,2) then
        if (npcs.base.functions.LangOK(originator,TradSpeakLang)==true) then
            ...
            if string.find(message,"[Rr]egel") or string.find(message,"[Rr]ule") then
                thisNPC:talk(CCharacter.say, npcs.base.functions.GetNLS(originator,"Ach so, ja. Die Regeln. Ein Spiel kostet dich 5 Kupfer. In diesem Säckchen sind Steine mit einer Nummer von 1 bis 10. Du ziehst sooft du möchtest.", 
                ...
                thisNPC:talk(CCharacter.say, npcs.base.functions.GetNLS(originator,"Erreichst du zusammengerechnet genau 11, bekommst du 15 Kupfer, kommst du über 11, verlierst du sofort. Bleibst du darunter und ziehst nicht weiter,",
                ...
                thisNPC:talk(CCharacter.say, npcs.base.functions.GetNLS(originator,"versuche ich dich zu überbieten, ohne über 11 zu kommen. Schaffe ich das nicht, gewinnst du 10 Kupfer.",
            ...
            if (string.find(message,"[Jj]a") or string.find(message,"[Yy]es")) and npcStatus[originator.id]==2 and npcTalksTo==originator.id then
                sayText=npcs.base.functions.GetNLS(originator,"Gut, versuch nochmal dein Glück!","Well, try again!");
                thisNPC:talk(CCharacter.say,sayText);
                doGamble(originator);
                lastTime=GetServerSeconds();
