...
function initializeNpc()
    ...
    npcs.base.autonpcfunctions.AddCycleText("#me scheppert mit einem Ledersäckchen","#me clangs with a small leather bag"); -- Fügt Text der alle paar Min gesagt wird hinzu
    npcs.base.autonpcfunctions.AddCycleText("Psst, Lust auf ein kleines Spielchen?","Psst, do you want to gamble a bit?");
...
function doGamble(user)         -- does all the gambling actions
    ...
    if npcStatus[user.id]==2 then        -- spiel zugestimmt
        sayText=npcs.base.npcautofunction.GetNLS(user,"Komm schon, zieh einen Stein!", "Come on, draw a stone!");
        ...
        --user:inform("1");
        talkString=npcs.base.npcautofunction.GetNLS(user,"Damit hast Du jetzt "..stoneSum[user.id]..". ","So you have "..stoneSum[user.id].." now. ");
        --user:inform("2");
        if (stoneSum[user.id]>11) then
            talkString=talkString.. npcs.base.npcautofunction.GetNLS(user,"Du hast mehr als 11 gezogen! Haha, du hast verloren!","You've drawn more than 11! Haha, you lost!");
        ...
        elseif (stoneSum[user.id]==11) then
            ...
            talkString=npcs.base.npcautofunction.GetNLS(user,getRandomElement(sayGerm),getRandomElement(sayEngl));
            ...
            talkString=talkString.. npcs.base.npcautofunction.GetNLS(user,"Willst du noch einmal ziehen?","Do you want to draw another stone?");
    ...
    if npcStatus[user.id]==3 then    -- npc muss ziehen
        ...
        repeat
            ...
            sayText=npcs.base.npcautofunction.GetNLS(user,"Ich habe "..drawnStone.." gezogen und insgesamt jetzt "..stoneSumNPC,"I have drawn "..drawnStone.." and summed up that makes "..stoneSumNPC);
            thisNPC:talk(CCharacter.say,sayText);
            if stoneSumNPC>11 then 
                finished=true;
                npcStatus[user.id]=0;
                sayText=npcs.base.npcautofunction.GetNLS(user,"Oh, ich habe verloren. Hier, dein Gewinn, 10 Kupferstücke.","Oh, I lost. Here's your reward: 10 copper coins.");
                thisNPC:talk(CCharacter.say, sayText);
            ...
            elseif stoneSumNPC>stoneSum[user.id] then
                finished=true;
                npcStatus[user.id]=0;
                sayText=npcs.base.npcautofunction.GetNLS(user,"Ich habe gewonnen!","I won!");
                thisNPC:talk(CCharacter.say, sayText);
                stoneSum[user.id]=0;
                npcTalksTo=0;
                stoneSumNPC=0;
    ...
    if npcs.base.autonpcfunctions.BasicNPCChecks(originator,2) then
        if (npcs.base.autonpcfunctions.LangOK(originator,TradSpeakLang)==true) then
            ...
            if (string.find(message,"[Ss]piel") or string.find(message,"[Pp]lay")) and npcStatus[originator.id]==0 and npcTalksTo==0 then
                ...
                thisNPC:talk(CCharacter.say, npcs.base.npcautofunction.GetNLS(originator,"Willst du mit mir spielen?","Do you want to play a game with me?"));
                npcStatus[originator.id]=1;
            end        
            if string.find(message,"[Rr]egel") or string.find(message,"[Rr]ule") then
                thisNPC:talk(CCharacter.say, npcs.base.npcautofunction.GetNLS(originator,"Ach so, ja. Die Regeln. Ein Spiel kostet dich 5 Kupfer. In diesem Säckchen sind Steine mit einer Nummer von 1 bis 10. Du ziehst sooft du möchtest.", 
                ...
                thisNPC:talk(CCharacter.say, npcs.base.npcautofunction.GetNLS(originator,"Erreichst du zusammengerechnet genau 11, bekommst du 15 Kupfer, kommst du über 11, verlierst du sofort. Bleibst du darunter und ziehst nicht weiter,",
                ...
                thisNPC:talk(CCharacter.say, npcs.base.npcautofunction.GetNLS(originator,"versuche ich dich zu überbieten, ohne über 11 zu kommen. Schaffe ich das nicht, gewinnst du 10 Kupfer.",
                ...
                if npcStatus[originator.id]==2 then
                    thisNPC:talk(CCharacter.say,npcs.base.npcautofunction.GetNLS(originator,"Na, willst du jetzt noch einen Stein ziehen oder nicht?",
                        "So, do you want to draw another stone or not?"));
                elseif npcStatus[originator.id]==0 and npcTalksTo==0 then
                    thisNPC:talk(CCharacter.say,npcs.base.npcautofunction.GetNLS(originator,"Na, Lust auf ein Spielchen?",
            ...
            if (string.find(message,"[Jj]a") or string.find(message,"[Yy]es")) and npcStatus[originator.id]==2 and npcTalksTo==originator.id then
                sayText=npcs.base.npcautofunction.GetNLS(originator,"Gut, versuch nochmal dein Glück!","Well, try again!");
                thisNPC:talk(CCharacter.say,sayText);
                doGamble(originator);
                lastTime=GetServerSeconds();
            ...
            if (string.find(message,"[Nn]ein") or string.find(message,"[Nn]o")) and npcStatus[originator.id]==2 and npcTalksTo==originator.id then
                ...
                sayText=npcs.base.npcautofunction.GetNLS(originator,"Na gut, wirf deine Steine wieder rein, jetzt bin ich dran!","Well, throw back the stones, it's my turn now!");
                thisNPC:talk(CCharacter.say,sayText);
                doGamble(originator);
                npcTalksTo=0;
                lastTime=0;
            ...
            if ((string.find(message,"[Jj]a") or string.find(message,"[Yy]es")) and npcStatus[originator.id]==1 and npcTalksTo==originator.id) then
                ...
                if npcs.base.autonpcfunctions.CheckMoney(originator,0,0,5) then    -- hat user das geld?
                    ...
                    sayText=npcs.base.npcautofunction.GetNLS(originator,"Du hast nicht genug Geld.","You don't have enough money.");
                    thisNPC:talk(CCharacter.say,sayText );
                    npcTalksTo=0;
                    npcStatus[originator.id]=0;
                end
            ...
            if (string.find(message,"[Nn]ein") or string.find(message,"[Nn]o")) and npcStatus[originator.id]==1 and npcTalksTo==originator.id then
                npcStatus[originator.id]=0;
                npcTalksTo=0;
                sayText=npcs.base.npcautofunction.GetNLS(originator,"Na gut, dann spielen wir eben nicht.", "Well then, we're not going to play.");
                thisNPC:talk(CCharacter.say, sayText);
            end
