...
local function clearPreviousBuild()
    for _, item in ipairs(itemList.items) do
        for _, category in ipairs(itemList.categories) do
            if M[tostring(category)..item.skill] then
                M[tostring(category)..item.skill] = nil
            end
        end
    end
    for _, tile in ipairs(itemList.tiles) do
        for _, category in ipairs(itemList.categories) do
            if M[tostring(category)..tile.skill] then
                M[tostring(category)..tile.skill] = nil
            end
        end
    end
end
