...
local function chooseWhatInfoToView(informedTarget, information, selectedTarget, spell)
    ...
    local callback = function(dialog)
        ...
        for targetIndex, target in ipairs(information) do
            if (target.type == "item" or target.type == "position" or isValidChar(target.target)) and tostring(target.target) == tostring(selectedTarget) then
...
function M.startDialogue(informedTarget, information, spell)
    ...
    local callback = function(dialog)
        ...
        for i = 1, #information do
            if index == i then
                M.alreadyOpenDialogue[informedTarget.id] = false
                if information[i].type == "item" or information[i].type == "position" or isValidChar(information[i].target) then
