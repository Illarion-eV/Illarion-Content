...
function M.callEffect(fireEffect, NPC)
    ...
    local _, lstNo = fireEffect:findValue("lastNumber")
    local _, fstNo = fireEffect:findValue("firstNumber")
    local i = fstNo
    local oldLstNo = lstNo
    ...
    while (i <= oldLstNo) and (i ~= 0) do
        ...
        local _, posX=fireEffect:findValue("fireX"..i)
        posX = posX - 100000
        local _, posY=fireEffect:findValue("fireY"..i)
        posY = posY - 100000
        local _, posZ=fireEffect:findValue("fireZ"..i)
        posZ = posZ - 100000

        local flamePos = position(posX, posY, posZ)
        if isFireOnField( flamePos ) then
            local flame = world:createItemFromId( 359, 1, flamePos, true, 999, 0)
            flame.wear = 1
        ...
        local _, next=fireEffect:findValue("fireN"..i)
        if fstNo == lstNo then
