	...
	if foundRoundCount then --Help	
	    if (roundCount/2) ~= math.floor(roundCount/2) then --Tutorial help
	        ...
	        if Character:getQuestProgress(309) < 2 and Character:isInRangeToPosition(position(37, 21, 100),7) then -- the char should walk to a specific postion (see: triggerfield/noobia_henry)
		        base.common.InformNLS(Character,"[Tutorial] Gehe zum Menschen am Ende des Piers um mit dem Tutorial zu beginnen. Klicke mit der rechten Maustaste auf ein Feld neben dem Menschen und halte die rechte Maustaste gedrückt. Alternativ kannst du deinen Charakter auch mit WASD, dem Ziffernblock oder den Pfeiltasten bewegen.","[Tutorial] To start the tutorial, please walk to the human at the end of the pier. To move, click and hold with the right mouse button on a spot close to the human. Alternatively, you can walk using the num pad, the arrow keys or WASD.");
	        end 
