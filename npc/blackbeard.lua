...
-- Insert Statements f�r N�tige NPCs
...
function dummy()
    if InitShips() then
        ...
        route = {4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4, -- 49x S�den
                 6,6,6,6,6,6,6,6,6,6,6,6,6,6,6,6,6,6,6,6,6,6,6,6,6,6,6,6,6,6,6,6,6,6,6,6,6,6,6,6,6,6,6,6,6,6,6,6,6,6,6,6, -- 52x Westen
                 4,4,4,4,4,4,4,4,4,4,4,4,4, -- 13x S�den
        ...
        --npc.base.functions.AddTraderTrigger("[Gg]r�[�s]+e","Aye. Was willst' Landratte?");  
    ...
    if (string.find(message,"[Ss]chiff") ~= nil) then
        if not CheckForPassanger(originator.id) then
            if (GetPassanger(1) == nil) then
                thisNPC:talk(CCharacter.say,"Aye. Du f�hrst mit und w�rdest zahlen. Noch wer?");
            else
                thisNPC:talk(CCharacter.say,"Aye. Du f�hrst mit. Noch wer?");
            end
            TryAddPassanger(originator);            
        else
            thisNPC:talk(CCharacter.say,"Du f�hrst doch schon mit! Arr. Verscheisser mich nich!");
        end
