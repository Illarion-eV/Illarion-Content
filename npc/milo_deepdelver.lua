...
--           Estralis Seborian                                                --
--                                                                            --
-- Last parsing: October 22, 2012                        easyNPC Parser v1.21 --
--------------------------------------------------------------------------------
...
talkEntry:addResponse("This stupid ox of mine doesn't move a single step and I am late returning to my family. Could you help me with my problem?");
...
talkEntry:addTrigger("mission");
talkEntry:addTrigger("Problem");
talkEntry:addResponse("Meine dummer Ochse bewegt sich kein Stück mehr nach vorne und ich bin jetzt schon spät dran, zu meiner Familie zurückzukehren. Könnt Ihr mir bei meinem Problem helfen?");
talkEntry:addConsequence(npc.base.consequence.quest.quest(106, "=", 1));
talkingNPC:addTalkingEntry(talkEntry);
end;
if (true) then
...
talkEntry:addResponse("My ox is doing anything for carrots, but a few miles ago they ran out. If you could get me ten carrots, I could show you a few tricks with herb lore, as I am quite proficient in this art.");
...
talkEntry:addResponse("Mein Ochse würde alles für Karotten machen aber vor ein paar Meilen sind sie mir ausgegangen. Wenn Ihr mir zehn Karotten besorgen könntet, könnte ich euch ein paar Tricks in Kräuterkunde beibringen, denn ich bin sehr bewandert in dieser Kunst.");
talkEntry:addConsequence(npc.base.consequence.quest.quest(106, "=", 2));
talkingNPC:addTalkingEntry(talkEntry);
end;
if (true) then
...
talkEntry:addResponse("Damn, you don't want to help me? What a pity! But please come back, if you want to lent me a hand with my ox!");
...
talkEntry:addResponse("Verdammt, ihr wollt mir nicht helfen? Wie schade! Aber bitte kommt wieder zurück zu mir, wenn ihr mir mit meinem Ochse zur Hand gehen wollt!");
talkEntry:addConsequence(npc.base.consequence.quest.quest(106, "=", 0));
talkingNPC:addTalkingEntry(talkEntry);
end;
if (true) then
...
talkEntry:addTrigger("Mission");
talkEntry:addTrigger("Quest");
talkEntry:addConsequence(npc.base.consequence.inform.inform("[Quest gelöst] Du erhälst einen Trank der Intelligenz, einen Trank der Konstitution und eine Flasche Elfenwein."));
...
talkEntry:addTrigger("Mission");
talkEntry:addTrigger("Quest");
talkEntry:addConsequence(npc.base.consequence.inform.inform("[Quest gelöst] Deine Fähigkeit in Kräuterkunde wurde gesteigert."));
talkEntry:addResponse("Ich danke euch vielmals für die Karotten. Nun werde ich kein Problem mehr haben, mit diesem faulen Bastard heim zu kehren. Kommt, lasst mich euch ein paar Dinge zu Kräuterkunde zeigen!");
talkEntry:addConsequence(npc.base.consequence.skill.skill(2, "herb lore", "+", 2));
talkEntry:addConsequence(npc.base.consequence.deleteitem.deleteitem(2493, 10, nil));
talkEntry:addConsequence(npc.base.consequence.quest.quest(106, "=", 3));
talkingNPC:addTalkingEntry(talkEntry);
...
talkEntry:addTrigger("Mission");
talkEntry:addTrigger("Quest");
talkEntry:addResponse("Aber das ist nicht die Anzahl an Karotten, die ich brauche. Mit den wenigen würde ich es niemals bis nach Hause schaffen. Bringt mir genau 10 Karotten und bitte beeilt euch!");
talkingNPC:addTalkingEntry(talkEntry);
end;
if (true) then
local talkEntry = npc.base.talk.talkNPCEntry();
...
talkEntry:addResponse("Dies ist ein gutes und respektables Handwerk. Man lernt eine Menge zu Kräutern umd anderen Ingredienzien, ihren Gebrauch und die Geschichte der Kräuterkunde.");
talkingNPC:addTalkingEntry(talkEntry);
end;
if (true) then
local talkEntry = npc.base.talk.talkNPCEntry();
talkEntry:addTrigger("herb");
talkEntry:addResponse("There are different kind of herbs which can be found on the ground. You just need a sickle in your hand to find them.");
talkingNPC:addTalkingEntry(talkEntry);
end;
if (true) then
local talkEntry = npc.base.talk.talkNPCEntry();
talkEntry:addTrigger("Kräuter");
talkEntry:addResponse("Es gibt verschiedene Arten von Kräutern, die auf der Erde gefunden werden können. Ihr benötigt nur eine Sichel in eurer Hand, um sie zu finden.");
talkingNPC:addTalkingEntry(talkEntry);
end;
if (true) then
local talkEntry = npc.base.talk.talkNPCEntry();
...
local talkEntry = npc.base.talk.talkNPCEntry();
talkEntry:addTrigger("Cadomyr");
talkEntry:addResponse("Cadomyrs Gebräuche sind eigenartig. Anstatt sich auf Weisheit und Wissen zu berufen, wird der Status einer Person in der GOchselschaft durch dessen Rang bestimmt. Es könnte sogar passieren, dass ein absoluter Volltrottel das Land beherrscht. Verrückt, nicht wahr?");
talkingNPC:addTalkingEntry(talkEntry);
end;
if (true) then
local talkEntry = npc.base.talk.talkNPCEntry();
...
talkEntry:addResponse("Ja, ich bin ein Halbling. Selbstverständlich das beste Volk weit und breit! Immer für einen netten Plausch zu haben und gute GOchselschafter. Man muss nur auf seinen Proviant aufpassen, wenn wir in der Nähe sind.");
talkingNPC:addTalkingEntry(talkEntry);
end;
if (true) then
local talkEntry = npc.base.talk.talkNPCEntry();
...
talkEntry:addResponse("Die Orks sind recht raue GOchselen. Ihr solltet keinen Streit mit ihnen anfangen.");
talkingNPC:addTalkingEntry(talkEntry);
end;
if (true) then
local talkEntry = npc.base.talk.talkNPCEntry();
...
talkEntry:addResponse("Mein Name ist Milo Deepdelver. Ich bin ein Kräuterkundiger aus Runewick. So die Götter es wollen, bin ich auf dem Weg nach Hause mit meinem unnützen Ochse hier!");
talkingNPC:addTalkingEntry(talkEntry);
end;
if (true) then
local talkEntry = npc.base.talk.talkNPCEntry();
...
talkingNPC:addCycleText("Nun komm schon, du störrischer alter Ochse!", "Come on, you stubborn old ox!");
talkingNPC:addCycleText("#me zieht verzweifelt am Halfter des Ochses.", "#me desperately pulls at the headcollar of the ox.");
...
talkingNPC:addCycleText("Ich verspreche dir, ich mach aus dir Ochseswurst, wenn du dich nicht auf der Stelle in Bewegung setzt.", "I promise I will turn you into ox sausage if you do not move right away.");
